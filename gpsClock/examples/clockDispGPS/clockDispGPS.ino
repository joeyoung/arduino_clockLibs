...
//#define LCD_ADR 0x21  // set the LCD address to 0x21
...
void setup( ) {
  ...
  byte errCode = clk.getTime( time, date );  // see 1/sec output
...
{
#ifdef GPSCLOCK_HDR
    clk.getTime( time, date );  // get both for gps
    if( time.sec == 0x00 ) {
      putTime( time );
      putDate( date );
      lcd.setCursor( 9, 1 );
