...
//          Mar 27/14 - use DS1307 software alarm implementation
//          Apr 7..9/14 - verify two alarms 
//          May  2/14 - DS1302 version
...
#ifdef DS1302_HDR
#define nrAlarms 2
DS1302 clk( 8, 9, 10 );     // pins connected to DS1302 clock, I/O, CE
byte alarm[ALARM_SIZE*2+2];   // for 'software alarm'
byte alarm1[ALARM_SIZE*2+2];  // second alarm scratch area
unsigned long timer[nrAlarms];  // separate timers for each alarm
#endif
#ifdef DS1307_HDR
#define nrAlarms 2
DS1307 clk;
...
#ifdef DS1307_HDR
  byte statflags;
  clk.getStatus( DS7ALMCTL, &statflags );
  statflags &= ~(1<<DSAF0); //clear alarm flag
  statflags &= ~(1<<DSAF1); // ..and other alarm flag
  clk.setStatus( DS7ALMCTL, statflags );
  alarm[2*ALARM_SIZE+1] = 0x80;   //arm checkAlarm for first entry
  alarm1[2*ALARM_SIZE+1] = 0x80;  // ..similarly, for second alarm
#endif
#ifdef DS1302_HDR
  byte statflags;
  clk.getStatus( DS2ALMCTL, &statflags );
  statflags &= ~(1<<DSAF0); //clear alarm flag
  statflags &= ~(1<<DSAF1); // ..and other alarm flag
  clk.setStatus( DS2ALMCTL, statflags );
  alarm[2*ALARM_SIZE+1] = 0x80;   //arm checkAlarm for first entry
  alarm1[2*ALARM_SIZE+1] = 0x80;  // ..similarly, for second alarm
#endif
}
...
void loop()
{
  ...
  pin12.flashOff( );
#ifdef DS1307_HDR
  if( clk.checkAlarm( 1, alarm1, timer ) ) pin12.flash( );  //single flash at instant of match
  if( clk.checkAlarm( 0, alarm, timer ) ) pin13.flash( );   //single flash at instant of match
#endif
#ifdef DS1302_HDR
  if( clk.checkAlarm( 1, alarm1, timer ) ) pin12.flash( );  //single flash at instant of match
  if( clk.checkAlarm( 0, alarm, timer ) ) pin13.flash( );   //single flash at instant of match
  ...
  if( sectick < millis( ) ) {
    ...
    if( alarm12 && time.sec <= 0x20 ) pin12.flash( );
    ...
    if( time.sec == 0x60 ) {
      ...
      lcd.print( "   1307" );
      byte statalm;
      clk.getStatus( DS7ALMCTL, &statalm );  //check alarm flag
      if( statalm & (1<<DSAF0) ) {
        clk.setStatus( DS7ALMCTL, statalm&(~(1<<DSAF0) ) );
        pin13.flash( );
        alarmFlashing = true;
      } else {
        alarmFlashing = false;
      } // if alarm0 flag bit set
      if( statalm & (1<<DSAF1) ) {
        clk.setStatus( DS7ALMCTL, statalm&(~(1<<DSAF1) ) );
        alarm12 = true;
      } else {
        alarm12 = false;
      }
      ...
      lcd.print( "   1302" );
      byte statalm;
      clk.getStatus( DS2ALMCTL, &statalm );  //check alarm flag
      if( statalm & (1<<DSAF0) ) {
        clk.setStatus( DS2ALMCTL, statalm&(~(1<<DSAF0) ) );
        pin13.flash( );
        alarmFlashing = true;
      } else {
        alarmFlashing = false;
      ...
      if( statalm & (1<<DSAF1) ) {
        clk.setStatus( DS2ALMCTL, statalm&(~(1<<DSAF1) ) );
        alarm12 = true;
      } else {
        alarm12 = false;
      }
