...
//	 	      Apr  8/14 - multiple alarms for DS1307
//          May  2/14 - DS1302 version
//          May  3/14 - Use burst mode clock setting
...
#ifdef DS1302_HDR
#define SECOND_ALARM
DS1302 clk( 8, 9, 10 );   // digital pins for DS1302 clock, data, CE
byte alarm[ALARM_SIZE*2];	// room for two alarms
#endif
#ifdef DS1307_HDR
#define SECOND_ALARM
DS1307 clk;
...
#ifdef DS1302_HDR
    clk.setTime( time, date );    // use burst write setting
#else
    clk.setTime( time );
#endif
...
//      alarm[ix] = ( (commline[lix]&0x0f)<<4) | (commline[lix+1]&0x0f);
  ...
  err = clk.getAlarm( 1, alarm+ALARM_SIZE );
  for( byte ix=ALARM_SIZE; ix<2*ALARM_SIZE; ix++ ) {
      Serial.print( alarm[ix], HEX );
      Serial.print( " " );
  }
  Serial.println( "" );
  ...
  if( ccount == 3*ALARM_SIZE ) {
    for( byte ix=0; ix<ALARM_SIZE; ix++ ) {
      byte lix = 3*ix;
      if( commline[lix] < 'A' ) {
        alarm[ix+ALARM_SIZE] = ( (commline[lix]&0x0f)<<4);
      } else {
        alarm[ix+ALARM_SIZE] = ( (commline[lix]&0x0f)+9)<<4;
      }
      if( commline[lix+1] < 'A' ) {
        alarm[ix+ALARM_SIZE] |= (commline[lix+1]&0x0f);
      } else {
        alarm[ix+ALARM_SIZE] |= ((commline[lix]&0x0f)+9);
      }
    } // for all alarm bytes
    clk.setAlarm( 1, alarm+ALARM_SIZE );
  } // if exactly right size for this alarm
